"""
Kafka helpers — consume inference_tasks, publish to inference_results.
"""
...
logger = logging.getLogger("inference-worker.kafka")
...
async def create_consumer() -> AIOKafkaConsumer:
    ...
    logger.info("Kafka consumer started on topic=%s", TOPIC_INFERENCE_TASKS)
    return consumer
...
async def publish_result(producer: AIOKafkaProducer, result: dict[str, Any]) -> None:
    ...
    logger.debug("Published prediction for session=%s frame=%s",
